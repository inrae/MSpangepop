...
Description: Class definitions for the graph structure used in the workflow.
This module defines the core classes for representing the variation graph, including:
- Node: Represents a segment of DNA sequence, stored as a bitarray for efficiency.
- Edge: Represents a directed connection between two nodes, with orientation information.
- Path: Represents a sequence of edges (a lineage) through the graph, with methods for manipulation such as bypassing, looping, inverting, swapping, and pasting nodes.
- Graph: Represents a variation graph, containing nodes, edges, and paths (lineages). 
"""
